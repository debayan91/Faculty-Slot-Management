...
import { collection, writeBatch, doc, type Firestore, getDocs } from 'firebase/firestore';
...
export async function seedScheduleTemplates(db: Firestore) {
  ...
  daysOfWeek.forEach((day) => {
    ...
    const allSlotsForDay = [...theoryForDay, ...labsForDay]
      .map((s) => {
  ...
  if (templatesCreated === 0) {
    return {
      success: true,
      message: 'All templates already exist. No new templates were created.',
    };
  }
  ...
  return {
    success: true,
    message: `Successfully created ${templatesCreated} new schedule templates.`,
  };
}
